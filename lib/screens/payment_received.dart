...
class _PaymentReceivedScreenState extends State<PaymentReceivedScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final monto = trabajo.solicitud.precio ?? 0;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Center(child: Text('Pago recibido',
                  ...
                  Center(
                    child: RichText(
                      ...
                      text: TextSpan(
                        style: const TextStyle(fontSize: 14, color: Colors.black54, fontFamily: 'Poppins'),
                        children: [
                          const TextSpan(text: 'Confirmaste que recibiste el pago\nde '),
                          // ← antes: '$25.000' hardcodeado
                          TextSpan(text: '\$${monto.toStringAsFixed(0)}',
                  ...
                  _buildOpcionMetodo(valor: 'efectivo', icon: Icons.money, texto: 'Recibí efectivo'),
                  const SizedBox(height: 8),
                  _buildOpcionMetodo(valor: 'transferencia', icon: Icons.swap_horiz, texto: 'Recibí transferencia'),
                  const SizedBox(height: 8),
                  ...
                  const SizedBox(height: 24),
                  // ← pasa monto y método al provider
                  _buildBotonConfirmar(context, monto),
                  const SizedBox(height: 20),
                ],
              ),
            ),
      ...
      child: SizedBox(
        ...
        child: Stack(
          ...
          children: [
            ...List.generate(8, (i) {
              final offsets = [
                const Offset(-75, -30), const Offset(75, -30), const Offset(-80, 10), const Offset(80, 10),
                const Offset(-55, 40), const Offset(55, 40), const Offset(-30, -45), const Offset(30, -45),
              ];
              final colors = [_amarillo, Colors.green, _amarillo, Colors.green, _amarillo, Colors.green, _amarillo, Colors.green];
  ...
  Widget _buildOpcionMetodo({required String valor, required IconData icon, required String texto}) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12),
            border: Border.all(color: seleccionado ? Colors.green : Colors.black12, width: seleccionado ? 2 : 1)),
        child: Row(children: [
          Icon(icon, size: 20, color: seleccionado ? Colors.green : Colors.black54),
          const SizedBox(width: 12),
          Text(texto, style: TextStyle(fontSize: 14, fontFamily: 'Poppins',
  ...
  Widget _buildOpcionPagoParcial() {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        const Expanded(child: Text('Pago parcial', style: TextStyle(fontSize: 14, fontFamily: 'Poppins'))),
        Switch(value: _pagoParcial, onChanged: (val) => setState(() => _pagoParcial = val),
  ...
  Widget _buildBotonConfirmar(BuildContext context, double monto) {
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          if (_metodoPago == null) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Seleccioná un método de pago.', style: TextStyle(fontFamily: 'Poppins')), backgroundColor: Colors.red),
            );
            return;
          }
          // ← confirma el pago en el provider (actualiza ganancias, mueve al historial)
          context.read<SolicitudProvider>().confirmarPago(
            monto: monto,
        ...
        child: const Text('Confirmar y continuar', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, fontFamily: 'Poppins')),
      ),
    );
  }
